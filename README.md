...
## Reading layer `PROVCenso2010' from data source `/home/geora/unidad-1-asignacion-1-datos-espaciales-en-r-geora2518/data/divisionRD.gpkg' using driver `GPKG'
...
``` r
plot(prov[prov$PROV %in% '08', 'PROV'])
```
...
## geometry type:  MULTIPOLYGON
## dimension:      XY
## bbox:           xmin: 465415.7 ymin: 2048450 xmax: 523595.5 ymax: 2105676
## epsg (SRID):    32619
## proj4string:    +proj=utm +zone=19 +datum=WGS84 +units=m +no_defs
...
``` r
miprov %>% dplyr::filter(PROV %in% '08')
## Simple feature collection with 1 feature and 4 fields
## geometry type:  MULTIPOLYGON
## dimension:      XY
## bbox:           xmin: 465415.7 ymin: 2048450 xmax: 523595.5 ymax: 2105676
...
## proj4string:    +proj=utm +zone=19 +datum=WGS84 +units=m +no_defs
##   PROV REG TOPONIMIA ENLACE                           geom
## 1   08  08  EL SEIBO   0808 MULTIPOLYGON (((523436.4 20...
prov %>% dplyr::filter(PROV %in% '08') %>% dplyr::select(areaenm2) %>% plot
```
...
``` r
prov %>% dplyr::filter(PROV %in% c('08', '02', '20')) %>% dplyr::select(areaenm2) %>% plot
```
...
``` r
mimun <- mun %>% dplyr::filter(PROV %in% '08')
mimun %>% plot
```
